    ...
    <header className="sticky top-0 z-50 border-b border-white/10 bg-steel-950/85 backdrop-blur">
      ...
      {open ? (
        <>
          ...
          <div className="fixed inset-x-4 top-[78px] z-50 mx-auto w-auto max-w-6xl sm:inset-x-6 sm:top-[86px] md:inset-x-8 md:top-[118px] lg:top-[154px]">
            <nav className="panel overflow-hidden rounded-2xl border border-white/20 bg-steel-900/95 p-2 shadow-2xl" aria-label="Site menu">
              {links.map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
                  onClick={() => setOpen(false)}
                  className="flex items-center justify-between rounded-xl px-4 py-3 text-sm font-semibold text-slate-100 transition hover:bg-white/10"
                >
